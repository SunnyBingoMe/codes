...
{
    #check 
    if [ -r $history_dir ]      #Èç¹û$history_dir ¶Ôµ±Ç°ÓÃ»§¿É¶Á
    then	
 	echo -n "You have installed qcd , overwrite it(y\Y or q\Q or n\N)? "
 	while read choice
 	do
	    if [ "$choice" = "y"  -o  "$choice" = "Y" ] #¸²¸Ç°²×°
	    then
	        break
	    fi	

	    if [ "$choice" = "q"  -o  "$choice" = "Q" ]  #Ê²Ã´Ò²²»×öÍË³ö
	    then
	        echo "Nothing to do!"
	        exit 1
	    fi
               
	    if [ "$choice" = "n"  -o  "$choice" = "N" ]  #°²×°µ½Ä¬ÈÏµÄÎ»ÖÃ
	    then
...
if [ $# -gt 1 ]    #Èç¹û´«ÈëµÄ²ÎÊý¸öÊý´óÓÚ1
then
    usage
fi 

case $1 in        #Èç¹û´«ÈëÒ»¸ö²ÎÊýÊÇdel
del)
    delete
