...
/*
  ...
  **************************************************************
  ...
  2012-02-28, refined by Sunny (BinSun@mail.com) to simplify the UI.
...
$rows     = isset($_POST['rows'])     ? $_POST['rows']     : $rowsDefault;
$columns  = isset($_POST['columns'])  ? $_POST['columns']  : $columnsDefault;
...
<form name="shell" enctype="multipart/form-data" method="post">
...
if (!$_SESSION['authenticated']) {
	...
	<fieldset style='border-width:0px' bgcolor="#FFFFCC">
		<?php 
		if (! $showeditor) { /* Outputs the 'terminal' without the editor */ ?>
			<div id="terminal">
			<textarea  bgcolor="#FFFFCC" name="output" readonly="readonly" cols="<?php echo $columns ?>" rows="<?php echo $rows ?>">
			<?php
			...
			<span style="float: right">
				...
				<?php //resize 
				if (! $showeditor) { /* You can not resize the textarea while the editor is 'running', because if you would
										   * do so you would lose the changes you have already made in the textarea since last saving */
					?>
					<!-- input type="submit" name="clear" value="Clear"-->
					<input type="text" name="rows" size="2" maxlength="3" value="<?php echo $rows ?>"> &times;
					<input type="text" name="columns" size="2" maxlength="3" value="<?php echo $columns ?>">
					<?php 
				} else { /* for 'editor-mode' */ ?>
		...
		<span class="pwd"><?php
			...
			if ( $showeditor ) {
				...
				for ($i=1; $i<count($parts); $i=$i+1) {
					echo '<a class="pwd" title="Change to this dir. (Your cmd will not be executed)" href="javascript:levelup(' 
					. (count($parts)-$i) . ')"><b>/</b></a>' ;
				...
				if (is_readable($_SESSION['cwd'])) { /* is the current directory readable? */
					...
					if (count($options)>0) {
						ksort($options);
						echo ' &nbsp;&nbsp; <a href="javascript:changesubdir()"> cd </a>';
						echo '<select name="dirselected">' . implode("\n", $options) . '</select>';
					}
				} else {
	...
	if ($ini['settings']['file-upload']) { ?>
		...
		<fieldset>
			Select file for upload:<input type="file" name="uploadfile" size="40"><input type="submit" value=" Upload ">
		</fieldset>
		<?php 
	} 
	?>
