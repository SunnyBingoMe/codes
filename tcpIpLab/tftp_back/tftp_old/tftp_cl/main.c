...
int     ini(char * argv_1, char * argv_2, char * argv_3);
...
int ini(char * argv_1, char * argv_2, char * argv_3)
{
    ...
    if(argv_1[0] == 'w')
    sprintf(cfig.homedir, "%s", argv_2);
    if(argv_1[0] == 'r')
    sprintf(cfig.homedir, "%s", argv_3);
	cfig.overwrite = 0;
	cfig.logLevel = 2;
...
/**/cfig.server = inet_addr("127.0.0.1");
	...
	if (cfig.sock == -1)
    {
	    printf("ERR ini() socket()\n");
        return 0;
    }
    #ifdef DEBUG
    printf("complete ini() create socket()\n");
    #endif
	...
	{
	    close(cfig.sock);
	    printf("ERR ini() failed to bind()\n");
		return 0;
	}
    #ifdef DEBUG
    printf("complete ini() bind()\n");
    #endif
...
void *hand_req(struct request * req)
{
    ...
    //socket(), bind(), connect() to ///cl.///remote
...
int main(int argc, char *argv[])
{
    ...
    int     rv;
    int     size_sockaddr, i_size_sockaddr;
    char   *p_ch1,*p_ch2;

    if( argc != 6)//no arg
        printf("complete no arguements,\n for command mod, use <w or r> </source file path>  </des dir path> <t or b> as arguements.\n");
    else//have arg
    {
        if( (argv[1][0] != 'w' && argv[1][0] != 'r') || (argv[4][0] != 't' && argv[4][0] != 'b') )//arg wrong
        {
            printf("ERR :arguments wrong, no argument accepted.\n");
            arg_set = 0;
        }
        else//arg right
        {
    ...
    if(arg_set == 0)
    {
        printf("complete ,please input mode ( w == write, r == read ):\n");
        scanf("%s",argv[1]);
        printf("complete ,please input source path (local begin with \"/\", remote begin with ipv4 address:\n");
        scanf("%s",argv[2]);
        printf("complete ,please input dest dir path (local begin with \"/\", remote begin with ipv4 address:\n");
        scanf("%s",argv[3]);
        printf("complete ,please input file type ( t == text/netascii/mail, b == octet/byte ):\n");
        scanf("%s",argv[4]);
        printf("complete ,please input file name:\n");
        scanf("%s",argv[5]);
        arg_set == 1;
    }
    if(argv[4][0] =='t')
...
*/
        ...
        if (arg_set == 1)////FD_ISSET(cfig.sock, &readfds))//confirm fd is really ready
        {
            ...
            errno = 0;
/*
            req->bytesReceived = recvfrom(cfig.sock,
                                          (char*)&(req->datain),
                                          sizeof(req->datain),
                                          0,
                                          (struct sockaddr *)&(req->client),
                                          &req->clientsize
                                         );
*/
            if(argv[1][0] == 'w')
            {
                req->bytesReceived = 2 + sizeof argv[2] + sizeof argv[5] + 1 + size_mode + 1;

                if(argv[4][0] =='t')
                    sprintf( (char*)&req->datain, "01%s%s0netascii",argv[2],argv[5]);
                else
                    sprintf( (char*)&req->datain, "01%s%s0octet",argv[2],argv[5]);
                temp = (char*) & (req->datain);
                temp += 2 + sizeof argv[2] + sizeof argv[5];
                temp = '\0';
                p_sub_slash = strstr(argv[3], sub_slash);
                in_size = ( (int)p_sub_slash - (int)argv[3] );
                strncpy(p_in_addr, argv[3], in_size);
                p_in_addr[in_size] = '\0';
                memset(&hints, 0, sizeof hints);
                hints.ai_family = AF_INET;
                hints.ai_socktype = SOCK_DGRAM;
                if ((rv = getaddrinfo(p_in_addr,
                                      "69",
                                      &hints,
                                      &servinfo
                                     )
                ...
                {
                    printf("ERR getaddrinfo()_w=>r\n");
                    exit(0);
                }
                #ifdef DEBUG
                else
                    printf("complete getaddrinfo()_w=>r\n");
                #endif
                size_sockaddr = sizeof (struct sockaddr);
                p_ch1 = (char *)&req->client;
                p_ch2 = (char *)servinfo->ai_addr;
                for(i_size_sockaddr = 0; i_size_sockaddr < size_sockaddr; i_size_sockaddr++)
                    *p_ch1 = *p_ch2;
            }
